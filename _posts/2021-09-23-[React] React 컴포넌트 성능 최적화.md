...
1. Props가 변경되었을 때
2. State가 변경되었을 때
3. 부모 컴포넌트가 렌더링되었을 때 (핵심)
4. (forceUpdate() 를 실행하였을 때) -> 직접 강제 렌더링


[data flow]

부모에서 state 값을 변경하게 되면 부모 컴포넌트가 리렌더링
-> 부모 컴포넌트가 리렌더링 되면서 해당 컴포넌트에 존재하는 모든 함수들이 새로 생성
-> 해당 함수들을 props로 전달받는 자식 컴포넌트들은 props의 변경을 감지하여 리렌더링


<br>

따라서 크게 위의 3가지 경우에 발생할 수 있는 뜻밖의 렌더링 경우를 모두 코드로 처리해야 `내가 원하는 상황에서만 해당 컴포넌트를 렌더링하여 view를 최적화 할 수 있다.` 만약 자식 컴포넌트와 상관도 없는 부분에서 부모 컴포넌트의 일부 text 값이 바뀌었을 때 자식 컴포넌트 또한 새로 만든다면 억울한 리렌더링이 될 수 있다. 아래 몇 가지 케이스들은 이러한 억울한 리렌더링 상황을 막을 수 있는 방법들이다.
핵심 포인트는 크게 2가지 부분이다. 

<br>

1. 부모 컴포넌트에서 무의미한 state, props의 변화를 발생시키지 않는것
2. 부모에서 리렌더링 되더라도 자식과 무관한 부분이라면 자식컴포넌트에서 리렌더링 되지 않도록 방지하는 것
